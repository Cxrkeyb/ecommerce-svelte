...
<div class="container mx-auto flex flex-col items-center justify-center py-4 px-4 sm:px-6 lg:px-8">
	{#if $cartStore.length > 0}
		<div class="w-full md:w-2/3 lg:w-1/2 bg-white rounded-lg shadow-lg p-6">
			<div class="flex flex-col gap-4">
				{#each $cartStore as item (item.id)}
					<div class="flex gap-4 items-center p-4 bg-gray-50 rounded-lg border">
						<img src={item.image} alt={item.name} class="w-20 h-20 object-cover rounded-lg" />
						<div class="flex-1">
							<h3 class="text-lg font-bold">
								{item.name.slice(0, 40).concat(item.name.length > 40 ? '...' : '')}
							</h3>
	...
	{:else}
		<div class="flex flex-col items-center justify-center bg-gray-100 p-20 rounded-lg shadow-md">
